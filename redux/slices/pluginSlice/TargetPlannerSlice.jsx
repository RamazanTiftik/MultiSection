...
import { doc, getDoc, setDoc, query, collection, Timestamp, getDocs, updateDoc, deleteDoc } from 'firebase/firestore';
...
export const deleteTargetPlan = createAsyncThunk(
    'targetPlan/deleteTargetPlan',
    async ({ userId, planId }, thunkAPI) => {
        try {
            const planRef = doc(db, 'users', userId, 'Target Plan', planId);
            await deleteDoc(planRef);
            return true;
        } catch (error) {
            console.error("Delete Target Plan Error:", error);
            return thunkAPI.rejectWithValue(error.message);
        }
    }
);
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(deleteTargetPlan.fulfilled, (state, action) => {
                ...
                state.targetedPlans = state.targetedPlans.filter(plan => plan.id !== deletedId);
